...
export default function AdminVerificaciones() {
  ...
  const [agePending, setAgePending] = useState<any[]>([]);
  ...
  async function loadPending() {
    ...
    setPending(photoData || []);

    const { data: ageData } = await supabase
      .from("profiles")
      .select("id, name, age, city, age_verification_photo_url, age_verification_status")
      .eq("age_verification_status", "pending");
    setAgePending(ageData || []);
  ...
  return (
    <div style={{ padding: 16, maxWidth: 480, margin: "0 auto" }}>
      ...
      <h2 style={{ fontSize: 18, marginBottom: 16 }}>Fotos de perfil pendientes</h2>

      {loading && <p style={{ color: "#9a9a9a" }}>Cargando...</p>}

      {!loading && pending.length === 0 && (
        <p style={{ color: "#9a9a9a", marginBottom: 32 }}>No hay fotos pendientes de revision.</p>
      )}
      ...
      <h2 style={{ fontSize: 18, marginTop: 32, marginBottom: 16, borderTop: "1px solid #2a2a2a", paddingTop: 24 }}>
        Documentos de identidad pendientes
      </h2>

      {!loading && agePending.length === 0 && (
        <p style={{ color: "#9a9a9a" }}>No hay documentos pendientes de revision.</p>
      )}

      {agePending.map((p) => (
        <div key={p.id} style={{ border: "1px solid #2a2a2a", borderRadius: 8, padding: 12, marginBottom: 16 }}>
          <p style={{ fontSize: 14, marginBottom: 8 }}>{p.name}, {p.age} - {p.city}</p>
          {p.age_verification_photo_url && (
            <img
              src={p.age_verification_photo_url}
          ...
          <div style={{ display: "flex", gap: 8 }}>
            <button
              onClick={() => handleAgeDecision(p.id, "approved")}
              disabled={actingOn === p.id}
              style={{ flex: 1, padding: 10, borderRadius: 8, border: "none", background: "#4bc97a", color: "#0a0a0a", fontWeight: 700 }}
            >
              Aprobar
            ...
            <button
              onClick={() => handleAgeDecision(p.id, "rejected")}
              disabled={actingOn === p.id}
              style={{ flex: 1, padding: 10, borderRadius: 8, border: "1px solid #e8352b", background: "none", color: "#e8352b" }}
            >
              Rechazar
